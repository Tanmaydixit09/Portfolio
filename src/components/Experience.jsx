...
const TimelineItem = ({ data, isCert, index }) => (
  ...
  >
    {/* Timeline dot */}
    <div className="md:hidden absolute left-[-5px] top-2 w-3 h-3 rounded-full bg-purple-500 shadow-[0_0_10px_#a855f7]"></div>
    
    <div className={`md:w-1/2 ${isCert ? 'md:ml-auto md:pl-12' : 'md:pr-12 md:text-right'} relative`}>
      {/* Desktop Timeline dot */}
      <div className={`hidden md:block absolute top-6 w-4 h-4 rounded-full bg-zinc-900 border-2 ${isCert ? 'border-blue-500 left-[-8px]' : 'border-purple-500 right-[-8px]'} z-20 transition-colors duration-300 hover:bg-${isCert ? 'blue' : 'purple'}-500`}></div>
      ...
      <div className="glass-card p-6 md:p-8 hover:border-zinc-700 transition-colors group relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <h3 className="text-xl md:text-2xl font-bold text-white mb-2 tracking-tight flex items-center gap-2 justify-start md:justify-${isCert ? 'start' : 'end'}">
